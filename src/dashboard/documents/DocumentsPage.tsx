...
export default function DocumentsPage() {
  ...
  useEffect(()=>{
    ...
    const go=async()=>{
      ...
      for(let i=first;i<pdfDoc.numPages;i++){
        setTimeout(async()=>{const c=pdfCanvasRefs.current[i];if(c)await renderPdfPage(pdfDoc,i+1,c);},(i-first+1)*300);
      }
    };
    setTimeout(go,80);
  ...
  useEffect(()=>{
    ...
    const go=async()=>{
      ...
      for(let i=first;i<docxPdfDoc.numPages;i++){
...
<div class="cert">
  ...
  const bakePages=async():Promise<string[]>=>{
    ...
    const pdoc   = isDocx ? docxPdfDoc        : pdfDoc;
    const total  = pdoc?.numPages || 0;
    if(total===0)return[];

    // Force-render any page that was lazily skipped so nothing is omitted
    for(let i=0;i<total;i++){
      const c=refs.current[i];
      if(c && c.width===0){ try{ await renderPdfPage(pdoc,i+1,c); }catch{} }
    }
    // Let the browser commit layout so getBoundingClientRect is accurate
    ...
    for(let i=0;i<total;i++){
      ...
      const px=base.width/canvasRect.width;
      ...
      for(const {ov,r,cs,ow,oh} of snapItems){
        ...
        const contentW=ow-padL-padR-bL-bR;
        const contentH=oh-padT-padB-bT-bB;
        // Offset of content-box top-left from element center (unrotated)
        const offLeft=-ow/2+bL+padL;
        ...
        if(ov.type==="image"||ov.type==="signature"){
          const img=await loadImg(ov.content);
          if(img){
            // Replicate object-fit:contain within the content box
            const ar=(img.naturalWidth||1)/(img.naturalHeight||1);
            let dw=contentW, dh=contentH;
            if(contentW/contentH>ar){ dh=contentH; dw=dh*ar; }
            else                    { dw=contentW; dh=dw/ar; }
            const dx=offLeft+(contentW-dw)/2;
            const dy=offTop +(contentH-dh)/2;
                        ...
                        <div key={ov.id} className={`dp-pdf-overlay-item${isLocked(ov)?" locked":""}`} style={{left:`${ov.x}%`,top:`${ov.y}%`}}
                          ...
                          {!isLocked(ov)&&(ov.type==="text"||ov.type==="date")&&(
                            <div style={{position:"absolute",bottom:-22,left:0,display:"flex",alignItems:"center",gap:3,background:"rgba(0,0,0,0.7)",borderRadius:4,padding:"2px 5px",zIndex:13}}>
                              ...
                              <button onMouseDown={e=>e.stopPropagation()} onClick={e=>{e.stopPropagation();setPdfOverlays(prev=>prev.map(o=>o.id===ov.id?{...o,rotation:((o.rotation||0)+15)%360}:o));setSaveStatus("unsaved");}}
                                style={{background:"none",border:"none",color:"#fff",fontSize:12,cursor:"pointer",padding:"0 2px"}}>↻</button>
                            </div>
                          )}
                        </div>
